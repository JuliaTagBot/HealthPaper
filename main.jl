...
# Setup the current environment on the just started workers
using Pkg
if Base.current_project() !== nothing
    project = dirname(Base.current_project())
    asyncmap(workers()) do p
        remotecall_wait(Pkg.activate, p, project)
    end
end

include("HealthPaper.jl")
